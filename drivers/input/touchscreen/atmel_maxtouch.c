...
static void force_release_pos(struct mxt_data *mxt)
{
...
static int __devinit mxt_stress_open(struct inode *inode, struct file *filp)
{
	printk("%s\n", __func__);
	  if( !atomic_dec_and_test(&touch_char_available) )
	{
...
static int __devinit mxt_stress_release(struct inode *inode, struct file *filp)
{
	printk("%s\n", __func__);
	atomic_inc(&touch_char_available); /* release the device */
	return 0;          /* success */
...
static int __devinit mxt_stress_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	struct mxt_data *mxt;
	u8 firmware_id[MXT_ID_BLOCK_SIZE];
	int err = 1;
...
static struct __devinit file_operations mxt_fops = {
	.owner =    THIS_MODULE,
