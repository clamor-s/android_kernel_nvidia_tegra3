...
static void fw_crc_checksum(u32* sum)
{
    u32 start_rom_address = FLASH_ROM_ADDRESS;
    u32 times = 0;
    u16 CRC = 0xFFFF;
    u32 write_size = FJM6MO_SECTOR_SIZE_1k;
    u32 rom[FJM6MO_SECTOR_SIZE_1k];
...
static char* get_fw_file_name(void)
{
    char *pFile;
    if(tegra3_get_project_id() == TEGRA3_PROJECT_TF700T)
...
static void fw_update_fail(void)
{
    update_mode = false;
...
static long sensor_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case SENSOR_CUSTOM_IOCTL_GET_FLASH_STATUS:
        {
            ...
            if (copy_to_user((void __user *)arg, &flash_status, sizeof(short)))
                return -EFAULT;
        ...
        case SENSOR_CUSTOM_IOCTL_GET_AE_LOCK:
        {
            ...
            if (copy_to_user((void __user *)arg, &ae_lock, sizeof(int)))
                return -EFAULT;
            break;
        }
        ...
        case SENSOR_CUSTOM_IOCTL_GET_AWB_LOCK:
        {
            ...
            if (copy_to_user((void __user *)arg, &awb_lock, sizeof(int)))
                return -EFAULT;
            break;
        }
...
static ssize_t i2c_camera(struct file *file, char const __user *buf, size_t count, loff_t *ppos)
{
    int len;
    int arg[2];
    int err;
...
static ssize_t i2c_calibration_shading(struct file *file, char const __user *buf, size_t count, loff_t *ppos)
{
    int len, wait_time;
...
static ssize_t i2c_calibration_pgain(struct file *file, char const __user *buf, size_t count, loff_t *ppos)
{
    int len;
    char light[10], cmd[10];
    int err;
...
static ssize_t i2c_set_cur_camera_id(struct file *file, char const __user *buf, size_t count, loff_t *ppos)
{
    int len;
...
static ssize_t i2c_camera_read(struct file *file, char const __user *buf, size_t count, loff_t *ppos)
{
    int len;
    u32 arg[3];
    u32 result = 0, temp;
...
static ssize_t i2c_camera_write(struct file *file, char const __user *buf, size_t count, loff_t *ppos)
{
    int len;
    u32 arg[4];
...
static ssize_t i2c_camera_read_memory(struct file *file, char const __user *buf, size_t count, loff_t *ppos)
{
    int len;
    u32 arg[4];
    u32 result = 0, temp = 0;
...
static ssize_t i2c_camera_write_memory(struct file *file, char const __user *buf, size_t count, loff_t *ppos)
{
    int len;
    u8 arg[5];
    u32 result = 0;
