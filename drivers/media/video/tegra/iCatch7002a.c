...
u32 I2C_7002DmemRd(u32 bankNum, u32 byteNum, u8* pbuf)
{
	u32 i, bank;
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	case SENSOR_IOCTL_SET_MODE:
	{
		...
		if (copy_from_user(&mode,(void __user *)arg,
	...
	case SENSOR_IOCTL_SET_CAMERA:
	{
		...
		//u16 testval;
		if (copy_from_user(&is_front_camera,(void __user *)arg,
	...
	case SENSOR_IOCTL_SET_COLOR_EFFECT:
	{
		if( tegra3_get_project_id() == TEGRA3_PROJECT_TF500T ||
			tegra3_get_project_id() == TEGRA3_PROJECT_ME301T ||
			tegra3_get_project_id() == TEGRA3_PROJECT_ME301TL) {
			if (copy_from_user(&coloreffect,(void __user *)arg,
				sizeof(coloreffect))) {
				return -EFAULT;
			}
	...
	case SENSOR_IOCTL_SET_WHITE_BALANCE:
	{
		...
		if (copy_from_user(&whitebalance,(void __user *)arg,
			sizeof(whitebalance))) {
			return -EFAULT;
		}
	...
	{
		u8 flash_led_type;
		if (copy_from_user(&flash_led_type,(void __user *)arg,
	...
	case SENSOR_CUSTOM_IOCTL_GET_FLASH_STATUS:
	{
		...
		if (copy_from_user(&flash_status,(void __user *)arg,
			sizeof(flash_status))) {
			return -EFAULT;
		}
		...
		if (copy_to_user((void __user *)arg, &flash_status, sizeof(flash_status)))
			return -EFAULT;
	...
	{
		u8 scene_mode;
		if (copy_from_user(&scene_mode,(void __user *)arg,
			sizeof(scene_mode))) {
			return -EFAULT;
		}
	...
	case SENSOR_CUSTOM_IOCTL_SET_AF_MODE:
	{
		...
		if (copy_from_user(&AF_cmd,(void __user *)arg,
			sizeof(AF_cmd))) {
			return -EFAULT;
		}
		switch(AF_cmd.cmd) {
	...
	case SENSOR_CUSTOM_IOCTL_GET_AF_MODE:
	{
		...
		copy_to_user((void __user *)arg, &AF_cmd, sizeof(AF_cmd));
		return 0;
#endif
		switch(AF_cmd.cmd) {
	...
	case SENSOR_CUSTOM_IOCTL_SET_EV:
	{
		...
		if (copy_from_user(&ev,(void __user *)arg, sizeof(short)))
			return -EFAULT;
	...
	case SENSOR_CUSTOM_IOCTL_GET_ISO:
	{
		...
		if (copy_to_user((void __user *)arg, &iso, sizeof(short)))
			return -EFAULT;
	...
	case SENSOR_CUSTOM_IOCTL_GET_ET:
	{
		...
		if (copy_to_user((void __user *)arg, &ET, sizeof(ET)))
		{
			return -EFAULT;
		}
	...
	case SENSOR_CUSTOM_IOCTL_SET_TOUCH_AF:
	{
		...
		if (copy_from_user(&touch_af,(void __user *)arg, sizeof(custom_touch_af_cmd_package)))
			return -EFAULT;
/*
...
*/
	...
	{
		u8 iso;
		if (copy_from_user(&iso,(void __user *)arg,
			sizeof(iso))) {
			return -EFAULT;
		}
		printk("SET_ISO as %d\n", iso);
	...
	{
		u8 flickering;
		if (copy_from_user(&flickering,(void __user *)arg,
			sizeof(flickering))) {
			return -EFAULT;
		}
	...
	case SENSOR_CUSTOM_IOCTL_GET_CAF_STATE:
	{
		...
		if (copy_to_user((void __user *)arg, &caf_state, sizeof(caf_state)))
			return -EFAULT;
	...
	{
		u8 continuous_af;
		if (copy_from_user(&continuous_af,(void __user *)arg,
	...
	case SENSOR_CUSTOM_IOCTL_SET_ICATCH_AE_WINDOW:
	{
		...
		if (copy_from_user(&ae_win,(void __user *)arg,
...
//			err = sensor_write_reg(info->i2c_client, 0x714E, 0x02);//the same with af window
	...
	case SENSOR_CUSTOM_IOCTL_SET_WDR:
	{
		if (tegra3_get_project_id() == TEGRA3_PROJECT_TF500T ||
			...
			u16 wdr_on;
			u16 dwdr_control;
			if (copy_from_user(&wdr_on,(void __user *)arg,
				sizeof(wdr_on))) {
				return -EFAULT;
			}
			wdr_on = wdr_on & 0x1;
	...
	case SENSOR_CUSTOM_IOCTL_SET_AURA:
	{
		if (tegra3_get_project_id() == TEGRA3_PROJECT_ME301T ||
			...
			if (coloreffect == YUV_ColorEffect_Aura) {
				u16 aura;
				if (copy_from_user(&aura, (void __user *)arg,
					sizeof(aura))) {
					return -EFAULT;
				}
	...
	{
		u32 ae_lock;
		if (copy_from_user(&ae_lock,(void __user *)arg,
			sizeof(ae_lock))) {
			return -EFAULT;
		}
		//ae_mode = ae_lock;
	...
	{
		u32 awb_lock;
		if (copy_from_user(&awb_lock,(void __user *)arg,
			sizeof(awb_lock))) {
			return -EFAULT;
		}
		//awb_mode = awb_lock;
	...
	{
		//int focus_mode;
		if (copy_from_user(&focus_control,(void __user *)arg,
	...
	case SENSOR_CUSTOM_IOCTL_FW_UPDATE_PROGRAM:
	{
		...
		if (copy_from_user(&rom_cmd,(void __user *)arg, sizeof(rom_cmd))) {
			fw_update_status = ICATCH_FW_UPDATE_FAILED;
			return -EFAULT;
		}
	...
	case SENSOR_CUSTOM_IOCTL_REG_SET:
	{
		...
		if (copy_from_user(&reg, (void __user *)arg, sizeof(register_setting)))
			return -EFAULT;
	...
	case SENSOR_CUSTOM_IOCTL_REG_GET:
	{
		...
		if (err)
			return err;
		reg.val = val;
		if (copy_to_user((void __user *)arg, &reg, sizeof(register_setting)))
			return -EFAULT;
		return 0;
	}
...
static ssize_t dbg_fw_update_write(struct file *file, char const __user *buf, size_t count,
				loff_t *ppos)
{
	char debug_buf[256];
	int cnt;
...
static ssize_t i2c_set_write(struct file *file, char const __user *buf, size_t count,
				loff_t *ppos)
{
	int len;
	int arg[2];
...
static ssize_t i2c_get_write(struct file *file, char const __user *buf, size_t count,
				loff_t *ppos)
{
	int len;
	int arg = 0;
...
static ssize_t dbg_iCatch7002a_chip_power_write(struct file *file, char const __user *buf, size_t count,
				loff_t *ppos)
{
	int len;
	int arg;
...
static ssize_t dbg_iCatch7002a_read_write_SPI_for_cal(struct file *file, char const __user *buf, size_t count,
				loff_t *ppos)
{
    char *bp = debugTxtBuf;
    int len, rw, index;
